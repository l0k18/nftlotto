...
// GetLotteryCount get the total number of lottery
...
// SetLotteryCount set the total number of lottery
...
// SetLottery set a specific lottery in the store
...
func (k Keeper) RemoveLottery(ctx sdk.Context, id uint64) {
	store := prefix.NewStore(ctx.KVStore(k.storeKey), types.KeyPrefix(types.LotteryKey))
	store.Delete(GetLotteryIDBytes(id))
}
