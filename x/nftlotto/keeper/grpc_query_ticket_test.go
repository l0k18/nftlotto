...
func TestTicketQuerySingle(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{
			desc:     "First",
			request:  &types.QueryGetTicketRequest{Id: msgs[0].Id},
			response: &types.QueryGetTicketResponse{Ticket: msgs[0]},
		},
		{
			desc:     "Second",
			request:  &types.QueryGetTicketRequest{Id: msgs[1].Id},
			response: &types.QueryGetTicketResponse{Ticket: msgs[1]},
		},
		{
			desc:    "KeyNotFound",
			request: &types.QueryGetTicketRequest{Id: uint64(len(msgs))},
